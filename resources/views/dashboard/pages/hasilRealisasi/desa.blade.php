...
<div class="row">
    <div class="col-12">
        <div class="form-group form-show-validation row p-0 mb-3">
            <div class="col-lg-12 col-md-12 col-sm-12">
                @component('dashboard.components.formElements.select',
                    [
                        'label' => 'Pilih Kecamatan',
                        'id' => 'kecamatan-desa-filter',
                        'name' => 'kecamatan_filter',
                        'class' => 'select2 filter-semua',
                    ])
                    @slot('options')
    ...
    <div class="col-lg-9 d-none col-peta">
        <div id="peta-desa"></div>
    </div>
    <div class="col-lg-3 pl-0 d-none col-info-penduduk">
        <div class="card border shadow-md">
            ...
            <div class="card-body">
                <small class="text-info fw-bold pilih-wilayah-dahulu" style="font-style: italic">Pilih Desa
                    di
                ...
                <div id="info-wilayah" class="info-wilayah d-none">
                    <ul class="list-group list-group-bordered">
                        <li class="list-group-item d-flex justify-content-between align-items-center px-3">
                            Desa:
                            <span class="font-weight-bold modal-text-desa">...........</span>
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center px-3">
                            Kecamatan:
    ...
    <div class="col mt-4 d-none card-tabel-desa">
        <div class="card mb-0 border shadow-md">
            <div class="card-header">
                <div class="card-title">Data Penduduk Yang Telah
                    Diintervensi Berdasarkan Desa <span class="wilayah-title"></span>
                    <span class="title-card-tahun d-inline fw-bold"></span>
                    <div class="card-tools float-right d-inline card-export-kecamatan-desa d-none">
                        <form action="{{ url('realisasi-intervensi/export-hasil-realisasi-desa') }}" method="POST"
                            ...
                            <input type="hidden" name="desa_filter" value="" class="desa-filter-export">
                            <button type="submit"
                                class="btn btn-secondary btn-border btn-round btn-sm mr-2 btn-export-kecamatan-desa d-none"
                                value="" name="" id="btnExportDesa">
                ...
                <div class="row">
                    <div class="col">
                        <small class="text-info fw-bold pilih-wilayah-dahulu" style="font-style: italic">Pilih
                            Desa di Map terlebih dahulu</small>
                    </div>
                    <div class="col-lg-12 col-md-12 mb-2">
                        @component('dashboard.components.formElements.select',
                            [
                                'label' => 'Sub Indikator',
                                'id' => 'sub-indikator-desa-filter',
                                'name' => 'sub_indikator_filter',
                                'class' => 'select2 filter filter-desa',
                            ])
                            @slot('options')
                    ...
                    <div class="col-lg-6 col-md-6">
                        @component('dashboard.components.formElements.select',
                            [
                                'label' => 'Status',
                                'id' => 'sasaran-intervensi-desa-filter',
                    ...
                    <div class="col mt-2">
                        <div class="table-responsive mt-2">
                            <table class="table table-hover table-striped table-bordered"
                                id="{{ $id ?? 'dataTablesDesa' }}" cellspacing="0" width="100%">
                                <thead>
...
@push('scripts')
    <script>
        function clearTabDesa() {
            $('#dataTablesDesa').DataTable().clear().destroy();
        ...
        $('#pills-tab-desa').click(function() {
            ...
            clearTabDesa()
            desa_kecamatan_id = $('#kecamatan-desa-filter').val()

            if (desa_kecamatan_id) {
                setTimeout(
                    function() {
                        $('#kecamatan-desa-filter').trigger("change")
                    }, 500
                );

            }

            $('#kecamatan-desa-filter').change(function() {
                desa_kecamatan_id = $(this).val()
                $('.col-info-penduduk').removeClass('d-none')
                $('.col-peta').removeClass('d-none')
                $('.card-tabel-desa').removeClass('d-none')

                initializeMapDesa(desa_kecamatan_id)
            })

            function initializeMapDesa(kecamatan_id) {
                        ...
                        if (response.status == 'success') {
                            for (var i = 0; i < response.data.length; i++) {
                                L.polygon(response.data[i].koordinatPolygon, {
                                    ...
                                    .on('click', L.bind(infoPendudukDesa, null, response.data[i]
                                        .id))
                            }
                        }
                    },
            ...
            function infoPendudukDesa(id) {
                desa = id
                kecamatan = desa_kecamatan_id
                $('.wilayah-title').removeClass('d-none')
                $('.btn-export-kecamatan-desa').removeClass('d-none')

                $('.kecamatan-filter-export').val(kecamatan)
                $('.desa-filter-export').val(id)
                ...
                $.get("{{ url('info-realisasi-desa') }}", {
                    desa_filter: desa,
                    kecamatan_filter: kecamatan,
                    tahun_filter: tahun
                }).done(function(response) {
                    $('.wilayah-title').text(response.nama_desa)
                    $('.modal-text-desa').text(response.nama_desa)
                ...
                var tableDesa = $('#dataTablesDesa').DataTable({
                    processing: true,
                    serverSide: true,
                    lengthMenu: [
                        [10, 25, 50, -1],
                    ...
                    ajax: {
                        ...
                        data: function(d) {
                            ...
                            d.status_filter = $('#sasaran-intervensi-desa-filter').val();
                            d.opd_filter = $('#opd-desa-filter').val();
                            d.desa_filter = desa;
                            d.sub_indikator_filter = $('#sub-indikator-desa-filter').val()
